...
function showResults(jsonObj)
{
	...
	var htmlSuggestions = '<table class="category tc"><thead><tr><th>Reference</th><th class="txt-left">Name</th><th>Stock</th></tr></thead>';

	this.suggestionList = jsonObj;

	for(var i=0; i<suggestions.length; i++)
		htmlSuggestions += '<tr class="suggestion" id="'+suggestions[i].id_product+'"><td>'+suggestions[i].reference+'</td><td class="txt-left">'+suggestions[i].value.replace(regex,'<span class="highlighted-text">'+this.queryStr+'</span>')+'</td><td>'+suggestions[i].stock+'</td></tr>';
