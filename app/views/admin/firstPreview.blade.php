...
{{ Form::open(array('url' => 'customer/suscription', 'method'=>'post')) }}
            ...
            <li class="item">
                ...
                <div class="check-in-out">
                    ...
                    {{Form::checkbox('voucher[]', $voucher->id, null, ['class' => 'voucher-checkbox'])}}
                    <label for="voucher">Apuntarme</label>
                </div>
                <figure>
                        <a href="{{url('customer/voucher/'.$voucher->id)}}">
                            @if(empty($voucher->img))
                                <img src="{{asset('images/default_voucher.jpg')}}" alt="voucher_image">
                            @else
                                <img src="{{asset('voucher_img/'.$voucher->img)}}" alt="voucher_image">
                            @endif
                            
                        </a>
                    </figure>

                    <h2><a href="{{url('customer/voucher/'.$voucher->id)}}" class="voucher-title">{{$voucher->title}}</a></h2>
                </div>
                <div class="bottom">
                    <p>{{$voucher->brief_description}}</p>
                    <hr>
                    ...
                    <hr>
                    <a class="btn btn-primary" href="{{url('customer/voucher/'.$voucher->id)}}">Learn more</a>
                </div>      
            </li>  
    </ul>
</div>
